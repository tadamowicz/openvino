...
bool InsertTransposeAfterConvOrPool::run_on_model(const std::shared_ptr<ngraph::Function>& f) {
    ...
    if (m_transpose_output) {
        std::shared_ptr<ov::Node> final_reshape_layer = f->get_ordered_ops()[f->get_ordered_ops().size() - 2];
        if (std::dynamic_pointer_cast<ngraph::opset7::Reshape>(final_reshape_layer) == nullptr) {
            THROW_GNA_EXCEPTION << "POSTPROC_OFF: Attempting to insert a transpose operation before 'final reshape' "
                                   "layer, but the layer does not exist in the graph";
        };

        std::shared_ptr<ov::Node> node = f->get_ordered_ops()[f->get_ordered_ops().size() - 4];

        log::debug() << "POSTPROC_OFF: Insert transpose operation into the graph before reshape node:"
                     << final_reshape_layer->get_friendly_name() << std::endl;
